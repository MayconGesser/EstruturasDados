...
#A ideia original era hashear o objeto inteiro, e nao soh a string do nome; porem a funcao interna 
#do Python, id(), gera um id novo para um mesmo objeto cada vez q o programa roda, acarretando um 
#comportamento inconsistente na tabela. Optou-se entao por usar as strings dos nomes
...
class TabelaHash:
	...
	def __init__(self, tamanho, objetos=None):	
		...
		#se uma lista de objetos foi passada no construtor constroi a tabela com os valores da lista
		if(objetos and isinstance(objetos, list)):
			
			for valor in objetos:
				
				self.processarEntrada(valor)
...
#cria lista a ser passada no construtor
listaDeProdutos = [produtoInserido.getNome(), produtoInserido2.getNome(), produtoInserido3.getNome(), produtoInserido4.getNome(), produto5.getNome()]

#cria objeto TabelaHash
tabela = TabelaHash(10, listaDeProdutos)
